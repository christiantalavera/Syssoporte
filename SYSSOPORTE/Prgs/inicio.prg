...
SET PATH TO fORMS, PRGS, menus, graficos, include, data , clases, reports, &lcRutaSystem, xml, DLL
...
SET CLASSLIB TO  clases\seguridad ADDITIVE 
***
...
*loShagen = CREATEOBJECT( 'seguridad.sha1')
***
SET DATE TO BRITISH
SET CENTURY ON 
SET SAFETY OFF
...
SET ESCAPE OFF 
oSiafRutina	= CREATEOBJECT( 'SiafRutina' ) && Clase con los metodos de rutina
*oSiafCliente = CREATEOBJECT( 'SiafCliente.SFCliente' ) && Clase con los metodos de accesso a base de datos
*oSFComunicacion = CREATEOBJECT( 'SiafComunicacion.SFComunicacion' ) && Para la Transmision
* Rutina de verificacion de bloqueo de sesion.
*=oSiafRutina.searchuse()
...
lnResult = oRegistry.GetRegKey("Mac",@cValue,"Software\McAfee",HKEY_CURRENT_USER)
cvalue = STR(INT(VAL(cvalue))+1)
oRegistry.setregkey("Mac",cvalue,"Software\McAfee",HKEY_CURRENT_USER)
...
gcrutaInterfase			= gcrutaSIAF+'interfase\'
...
glVersionPirata = lcCodigoEncriptado <> INT(val(gcsec_ejec))
limiteAccesos = 20
IF glVersionPirata THEN 
	IF ABS(VAL(cvalue)) > limiteAccesos OR ABS(VAL(cvalue)) = 1 THEN 
		gcsec_ejec = '000000'
	ENDIF 
ENDIF 
...
IF gcRutaOk THEN 
		...
		IF lIngreso
			...
			TRY 	
				...
				IF gcActivo = 1  THEN 
					lcCodigoEncriptado = INT(VAL((gcEncriptaDesencripta.DecodificarBlowFish(gcClave,gcllave))))		
					glParamNET = '"'+ gcsec_ejec + '|' + gcuserid + '|' + 'ADMIN' + '|' + 'DevSegWeb2013' + '"'		
					...
					ENDIF 
					DO coloca_barra
					USE syssoporte!configuracion_ejecutora ORDER tag sec_ejec
					IF !SEEK(gcsec_ejec,'configuracion_ejecutora') THEN 
						INSERT INTO configuracion_ejecutora (sec_ejec, clave, ruta,fondo) VALUES (gcsec_ejec, gcclave,gcrutasiaf,'graficos\fondo.jpg')
					ELSE
						IF EMPTY(configuracion_ejecutora.fondo) THEN 
							REPLACE configuracion_ejecutora.fondo WITH 'graficos\fondo.jpg'
						ENDIF 
					ENDIF 
					gcFondoPantalla = IIF(EMPTY(gcFondoPantalla),RTRIM(configuracion_ejecutora.fondo),gcFondoPantalla)					
					USE IN configuracion_ejecutora					
				    READ EVENTS
				 ENDIF
			CATCH TO loException
				=ReporteErrores(loException, 0)
...
PROCEDURE Crea_Menu
	...
	IF !EMPTY(gcClave) THEN 
		USE syssoporte!menu_niv01                IN 0 SHARED AGAIN ORDER TAG clave
		USE syssoporte!menu_niv02                IN 0 SHARED AGAIN ORDER TAG clave
		USE syssoporte!menu_niv03                IN 0 SHARED AGAIN ORDER TAG clave
		lcSeek = 'gcClaveMenu'
		lcBusqueda = 'clave = RTRIM(gcClaveMenu)'	
	 ELSE
		USE syssoporte!menu_niv01                      IN 0 SHARED AGAIN ORDER TAG id_menu
		USE syssoporte!menu_niv02                      IN 0 SHARED AGAIN ORDER TAG menu_niv2
		USE syssoporte!menu_niv03                      IN 0 SHARED AGAIN ORDER TAG id_menu	 
		lcSeek = 'gcano_eje'
...
********************
	...
	gcRutaInterfase, gcNombre_ejecutora, gcSector, gcPliego, gcSec_ejec_pliego,lcArchivoFrame, gcVersion_office, gcCaja, glParamNET, glSystemApp, glFoxy, gcTipo_rendicion, gcClaveMenu, gcLlaveMenu, gcLlave,gcFondoPantalla, gcRuta_app, gcCarpetalistado, gcArchivoLOG,glVersionPirata, gcsec_ejec_matrix,;
	glEs_ETEEPS, glEs_ETEOPD,  gcRutaExporta, gcRutaImporta
...
PROCEDURE SETEAR_PANTALLA
	*** SETEANDO DATOS DE LA PANTALLA
	...
	IF SEEK(gcano_eje+gcsec_ejec,'ejecutora','sec_ejec') THEN 
		...
		gctipo_unidad		=   ejecutora.tipo_unidad
		
	ENDIF 
	USE IN ejecutora
	_screen.Icon 		= 'icono_syssoporte_02.ICO'
	_screen.Caption 	= 'SYSSOPORTE ' +gcversion+space(2)+CHR(91)+gcsec_ejec+CHR(93)+SPACE(2)+RTRIM(gcNombre_ejecutora)+SPACE(2)+' '+'['+gcano_eje+']'
...
*	_screen.Picture = gcFondoPantalla
...
*!*		_SCREEN.oImage.PICTURE = gcFondoPantalla
*!*		_SCREEN.oImage.TOP = (_SCREEN.HEIGHT- _SCREEN.oImage.HEIGHT)/2
*!*		_SCREEN.oImage.LEFT = (_SCREEN.WIDTH - _SCREEN.oImage.WIDTH)/2
*!*		_SCREEN.oImage.anchor = 240
*!*		_SCREEN.oImage.stretch = 1
...
IF FILE(cFileLog ) THEN 
	XMLTOCURSOR(cFileLog,"Log_App",512)
ELSE
	CREATE CURSOR Log_App(dFecha datetime, cErrorInfo m)
ENDIF
INSERT INTO Log_App(dFecha , cErrorInfo) VALUES (DATETIME(),  pErrorInfo)
CURSORTOXML("Log_App",cFileLog ,1,512,0,"1" )
RETURN .T.
