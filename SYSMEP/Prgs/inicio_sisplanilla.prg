...
SET SYSMENU OFF
ON SHUTDOWN DO SalirSistema IN inicio_sisplanilla.prg
				...
				IF gcActivo = 1  THEN 
					...
					DO CASE 
						CASE ALLTRIM(gcuserid) = 'ADMINIST'
							DO menu_administrador_sisplanilla.mpr
							llOk = .T.
						CASE ALLTRIM(gcuserid) = 'MATRIX'						
							DO menu_master_planilla.mpr					
							llOk = .T.
						OTHERWISE 
...
********************
	...
	*-- Instrucciones DECLARE DLL para leer/escribir en archivos INI privados-
	...
	gcOldEscape="ON"
	gcOldTalk="ON"
	gcFileIni = 'sisplanilla.ini'
	gcSession = "PARAMETROS DEL SISPLANILLA"
	gcnom_fuente = 'RUBRO'
...
PROCEDURE SETEAR_PANTALLA
	*** SETEANDO DATOS DE LA PANTALLA
	...
	_screen.Caption 	= 'SISPLANILLA ' +gcversion+space(2)+CHR(91)+gcsec_ejec+CHR(93)+SPACE(2)+gcNombre_ejecutora
