...
gcAmbito 				= ''
gcsiglas 				= ''
gcTipo_reporte 			= STR(INT(VAL(gcTipo_reporte)),1,0)
USE sysmep!configuracion_ejecutora IN 0 ORDER tag sec_ejec
IF SEEK(gcsec_ejec,'configuracion_ejecutora') THEN 
	gcsiglas = configuracion_ejecutora.siglas
ENDIF 
...
*!*	USE SYSMEP!convenio_proyecto IN 0 
*!*	SELECT proyecto, organismo FROM convenio_proyecto WHERE proyecto <> '000' INTO CURSOR curTmp
*!*	SELECT curTmp
*!*	COUNT ALL TO x
*!*	GO TOP 
*!*	IF x=1 THEN 
*!*		gcProyecto = curTmp.proyecto
*!*		gcorganismo = curTmp.organismo
*!*	ENDIF 
*!*	USE IN curTmp	
*!*	USE IN convenio_proyecto
...
PROCEDURE Crea_Menu
	IF !EMPTY(gcClave) THEN 
		USE SYSMEP!menu_niv01                IN 0 SHARED AGAIN ORDER TAG clave
		USE SYSMEP!menu_niv02                IN 0 SHARED AGAIN ORDER TAG clave
		USE SYSMEP!menu_niv03                IN 0 SHARED AGAIN ORDER TAG clave
		lcSeek = 'gcClaveMenu'
		lcBusqueda = 'clave = RTRIM(gcClaveMenu)'	
	 ELSE
		USE SYSMEP!menu_niv01                      IN 0 SHARED AGAIN ORDER TAG id_menu
		USE SYSMEP!menu_niv02                      IN 0 SHARED AGAIN ORDER TAG menu_niv2
		USE SYSMEP!menu_niv03                      IN 0 SHARED AGAIN ORDER TAG id_menu	 
		lcSeek = 'gcano_eje'
	   ...
	   SELECT * FROM menu_niv02 ;
	    ...
	    SELECT menu_niv01
	              ...
	              Define Popup  &lcNamePopup   Margin Relative Shadow Color Scheme 4
	                        ...
	                        i=i+1
	                        =SEEK(Cur_menu_niv02.ano_eje+Cur_menu_niv02.sec_ejec+p_user + m.modulo+m.id_menuniv01+Cur_menu_niv02.ID_MENUNIV02,"menu_niv02","MENU_NIV2")
	                         ...
	                         IF menu_niv02.tipo_nodoniv2="S" THEN                          
	                             ...
	                             lcNamePopup2 = lcNamePopup + "_" + padl(INT(i),2,'0')
	                              ...
	                              SCAN WHILE i3<=lnCountOpc3
	                                    ...
	                                    =SEEK(Cur_menu_niv03.ano_eje+Cur_menu_niv03.sec_ejec+p_user + m.modulo+ m.id_menuniv01+menu_niv02.id_menuniv02+Cur_menu_niv03.ID_MENUNIV03,"menu_niv03","id_menu")
	                                    i3=i3+1
...
********************
	...
	*-- Instrucciones DECLARE DLL para leer/escribir en archivos INI privados-
	...
	gcSession = "PARAMETROS DEL SYSMEP"
	gcnom_fuente = 'RUBRO'
...
PROCEDURE SETEAR_PANTALLA
	*** SETEANDO DATOS DE LA PANTALLA
	...
	ENDIF 
	USE IN ejecutora
	_screen.Icon 		= 'sysmep_G6T_icon.ICO'
	_screen.Caption 	= 'SYSMEP ' +gcversion+space(2)+CHR(91)+gcsec_ejec+CHR(93)+SPACE(2)+RTRIM(gcNombre_ejecutora)+SPACE(2)+' '+'['+gcano_eje+']'
...
*!*					CASE 'CONVENIO' $ UPPER(lcString)
*!*						gcProyecto = SUBSTR(lcString,AT('=',lcString)+1)	
...
STORE lcruta+'\EXPORTA'  TO ladirectory(3)
STORE lcruta+'\IMPORTA'  TO ladirectory(4)
STORE lcruta+'\GRAFICOS' TO ladirectory(5)
...
IF FILE(cFileLog ) THEN 
	XMLTOCURSOR(cFileLog,"sysmep_log",512)
ELSE
	CREATE CURSOR sysmep_log(dFecha datetime, cErrorInfo m)
ENDIF
INSERT INTO sysmep_log(dFecha , cErrorInfo) VALUES (DATETIME(),  pErrorInfo)
CURSORTOXML("sysmep_log",cFileLog ,1,512,0,"1" )
RETURN .T.
